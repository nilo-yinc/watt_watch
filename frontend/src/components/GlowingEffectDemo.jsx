...
const GridItem = ({ area, icon, title, description }) => {
    return (
        <li className={`min-h-[14rem] list-none ${area}`}>
            <div className="relative h-full rounded-2xl border border-cyan-500/10 p-2 md:rounded-3xl md:p-3">
                <GlowingEffect
                    spread={40}
                    glow={true}
                    disabled={false}
                    ...
                    inactiveZone={0.01}
                />
                <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl p-6 md:p-6 bg-black/60 backdrop-blur-sm shadow-[0px_0px_27px_0px_#0c4a6e33]">
                    <div className="relative flex flex-1 flex-col justify-between gap-3">
                        <div className="w-fit rounded-lg border border-cyan-500/30 p-2 bg-cyan-500/5">
                            {icon}
                        </div>
                        <div className="space-y-3">
                            <h3 className="pt-0.5 font-display text-xl/[1.375rem] font-semibold text-balance text-white md:text-2xl/[1.875rem] tracking-wide">
                                {title}
                            </h3>
                            <p className="font-sans text-sm/[1.125rem] text-slate-400 md:text-base/[1.375rem]">
                                {description}
                            </p>
                        </div>
                    </div>
