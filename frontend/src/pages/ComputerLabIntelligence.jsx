        ...
        <Spotlight className="min-h-full">
            ...
            <div className="relative z-10 max-w-[1400px] mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <h1 className="text-2xl font-bold text-[var(--ww-text-1)] tracking-tight mb-1">Computer Lab Monitoring</h1>
                    <p className="text-xs font-mono text-[var(--ww-text-3)]">Desktop array analysis · Hidden waste detection</p>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-8">
                    {[
                        ...
                        { label: 'CURRENTLY ON', val: totalOn, sub: `${((totalOn / totalDesktops) * 100).toFixed(0)}% utilization`, accent: 'text-[var(--ww-text-1)]' },
                        ...
                        <motion.div key={i} className="hud-card p-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                            ...
                            {s.sub && <div className="text-[9px] font-mono text-[var(--ww-text-muted)] mt-1">{s.sub}</div>}
                        ...
                        <motion.div key={lab.id} className={`hud-card ${lab.hiddenWaste ? 'border-red-500/15' : ''}`}
                            ...
                            <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.03]">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <span className="text-sm font-semibold text-[var(--ww-text-1)]">{lab.name}</span>
                                </div>
                                {lab.hiddenWaste && (
                            ...
                            <div className="p-5">
                                <div className="text-[10px] font-mono text-[var(--ww-text-muted)] mb-4">{lab.building}</div>

                                {/* Metrics */}
                                <div className="grid grid-cols-3 gap-2 mb-4">
                                    {[
                                        { l: 'OCCUPANTS', v: lab.occupancy, a: lab.occupancy === 0 ? 'text-red-400' : 'text-cyan-400' },
                                        { l: 'PCS ON', v: `${lab.desktopsOn}/${lab.totalDesktops}`, a: 'text-[var(--ww-text-1)]' },
                                        { l: 'WASTE', v: `${lab.energyWaste} kWh`, a: 'text-amber-400' },
                                    ].map((m, j) => (
                                        <div key={j} className="bg-white/[0.02] rounded-md p-2.5">
                                            <div className="text-[8px] font-mono text-[var(--ww-text-muted)] mb-0.5">{m.l}</div>
                                ...
                                {/* Desktop Analysis */}
                                <div className="bg-white/[0.01] border border-[var(--ww-border)] rounded-md p-4 mb-4">
                                    <div className="text-[9px] font-mono text-cyan-400 tracking-wider mb-3">DESKTOP ARRAY ANALYSIS</div>
                                    <div className="grid grid-cols-3 gap-3 text-center">
                                        {[
                                            { v: lab.desktopsOn, l: 'POWERED', a: 'text-[var(--ww-text-1)]' },
                                            ...
                                            <div key={j}>
                                                ...
                                                <div className="text-[8px] font-mono text-[var(--ww-text-muted)] mt-0.5">{d.l}</div>
                                            </div>
                                        ))}
                                    </div>
                                    {lab.monitorsOff > 5 && (
                                        <div className="mt-3 pt-3 border-t border-[var(--ww-border)] flex items-center gap-2">
                                ...
                                <div className="flex items-center gap-3 mb-4">
                                    <span className="text-[9px] font-mono text-[var(--ww-text-muted)] w-16">EFFICIENCY</span>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {[
                            ...
                            <div key={i} className="bg-white/[0.01] rounded-md p-3 border border-white/[0.03]">
                                ...
                                <p className="text-[10px] font-mono text-[var(--ww-text-3)] leading-relaxed">{tip.desc}</p>
                            </div>
                        ))}
                    </div>
                </div>
