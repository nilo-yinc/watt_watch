            ...
            <div className="max-w-[1200px] mx-auto">
                <div className="flex items-start justify-between mb-8">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-[var(--ww-text-1)] tracking-tight mb-1">Power Control</h1>
                        <p className="text-xs font-mono text-[var(--ww-text-3)]">Direct appliance control · Safety interlocks active</p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                    ...
                    <div className="hud-card p-5">
                        ...
                        <select value={selectedRoom} onChange={(e) => setSelectedRoom(e.target.value)}
                            className="w-full px-3 py-2.5 bg-transparent border border-[var(--ww-border)] rounded-md text-[var(--ww-text-2)] text-xs font-mono focus:border-cyan-500/30 focus:outline-none mb-4">
                        ...
                        {selectedRoomData && (
                            <div className="space-y-2 pt-3 border-t border-[var(--ww-border)]">
                                {[
                                    { l: 'Type', v: selectedRoomData.type },
                                    { l: 'Building', v: selectedRoomData.building },
                                ].map((d, i) => (
                                    <div key={i} className="flex justify-between"><span className="text-[10px] font-mono text-[var(--ww-text-muted)]">{d.l}</span><span className="text-xs font-mono text-[var(--ww-text-1)]">{d.v}</span></div>
                                ))}
                                <div className={`flex justify-between items-center py-2 px-3 rounded-md mt-2 ${selectedRoomData.occupancy > 0 ? 'bg-red-500/[0.06] border border-red-500/15' : 'bg-emerald-500/[0.06] border border-emerald-500/15'}`}>
                                    <span className="text-[10px] font-mono text-[var(--ww-text-2)]">Occupancy</span>
                    ...
                    <div className="hud-card p-5">
                        ...
                        {!selectedRoom ? (
                            <div className="flex items-center justify-center py-16"><span className="text-xs font-mono text-[var(--ww-text-muted)]">SELECT A TARGET</span></div>
                        ) : !canTakeAction ? (
                            <div className="p-4 bg-red-500/[0.04] border border-red-500/15 rounded-md">
                                ...
                                <p className="text-[10px] font-mono text-[var(--ww-text-3)]">Controls locked — {selectedRoomData.occupancy} occupants detected</p>
                            </div>
                        ) : (
                            <div className="space-y-3">
                                {[
                                    ...
                                    <button key={i} onClick={() => handleAction(btn.action)}
                                        ...
                                        <div className="text-[10px] font-mono text-[var(--ww-text-muted)]">{btn.sub}</div>
                                    </button>
                                ))}
                            </div>
                        )}
                    ...
                    <div className="hud-card p-5 mb-6">
                        ...
                        <div className="grid grid-cols-4 gap-3">
                            {[
                                ...
                                <div key={i} className={`p-3 rounded-md border ${a.on ? 'border-amber-500/15 bg-amber-500/[0.03]' : 'border-[var(--ww-border)]'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        {['Occupancy interlock prevents live-room shutoff', 'All actions timestamped and audit-logged', 'Confirmation required for all power changes', 'Emergency restore available via admin terminal'].map((t, i) => (
                            <div key={i} className="flex items-start gap-2 p-3 bg-white/[0.01] rounded-md border border-white/[0.03]">
                                ...
                                <span className="text-[10px] font-mono text-[var(--ww-text-2)] leading-relaxed">{t}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="hud-card p-6 max-w-sm w-full mx-4">
                        ...
                        <p className="text-sm text-[var(--ww-text-2)] mb-1">
                            {actionType === 'powerOff' ? 'Turn OFF' : 'Restore'} power in:
                        </p>
                        <p className="text-sm font-mono font-bold text-[var(--ww-text-1)] mb-4">{selectedRoomData?.name}</p>
                        <div className="flex gap-2">
                            <button onClick={() => setShowModal(false)} className="flex-1 py-2 text-xs font-mono text-[var(--ww-text-2)] border border-[var(--ww-border)] rounded-md hover:bg-white/[0.02]">CANCEL</button>
