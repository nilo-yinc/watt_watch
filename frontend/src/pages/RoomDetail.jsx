...
export default function RoomDetail() {
        ...
        return (
            <div className="min-h-full flex items-center justify-center">
                <div className="text-center">
                    <div className="text-slate-600 text-6xl font-mono mb-4">404</div>
                    <p className="text-slate-500 text-sm font-mono mb-6">FEED NOT FOUND</p>
    ...
    const getStatus = (s) => {
        switch (s) {
            ...
            default: return { dot: 'bg-slate-400', label: 'UNKNOWN', text: 'text-slate-400' };
        }
    };
        ...
        <Spotlight className="min-h-full">
            ...
            <div className="relative z-10 max-w-[1200px] mx-auto">
                ...
                <div className="flex items-center gap-2 mb-6">
                    <Link to="/campus" className="text-slate-600 text-xs font-mono hover:text-cyan-400 transition-colors">CAMPUS</Link>
                ...
                <div className="flex items-start justify-between mb-8">
                    <div>
                        ...
                        <h1 className="text-2xl font-bold text-white tracking-tight mb-1">{room.name}</h1>
                        <p className="text-slate-500 text-xs font-mono">{room.type} · {room.building} · {room.monitoring}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3 mb-6">
                    {[
                        ...
                        { label: 'UTILIZATION', value: `${((room.occupancy / room.capacity) * 100).toFixed(0)}%`, accent: 'text-white' },
                    ...
                    <div className="col-span-2 hud-card p-5">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {[
                                ...
                                <div key={i} className={`p-3 rounded-lg border ${a.on ? 'border-amber-500/15 bg-amber-500/[0.03]' : 'border-white/[0.04] bg-white/[0.01]'}`}>
                                    ...
                                    <div className={`text-[9px] font-mono font-bold mt-1 tracking-wider ${a.on ? 'text-amber-400' : 'text-slate-700'}`}>{a.on ? 'ACTIVE' : 'INACTIVE'}</div>
                                </div>
                            ))}
                            {room.appliances.desktops > 0 && (
                                <div className="p-3 rounded-lg border border-cyan-500/15 bg-cyan-500/[0.03] col-span-3">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <span className="text-xs font-mono font-bold text-white">DESKTOP ARRAY</span>
                                            <div className="text-[10px] font-mono text-slate-500 mt-0.5">{room.appliances.desktops} units active</div>
                                        </div>
                    ...
                    <div className="hud-card p-5">
                        ...
                        <div className="space-y-2">
                            {roomTimeline.length > 0 ? roomTimeline.map((event, i) => (
                                <motion.div key={i} className="flex items-start gap-2 p-2 rounded-md bg-white/[0.01] border border-white/[0.03]"
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[9px] font-mono text-slate-600">{formatTime(event.time)}</p>
                                    </div>
                                </motion.div>
                            )) : (
                                <p className="text-xs text-slate-700 font-mono text-center py-4">NO RECENT ACTIVITY</p>
                            )}
                        </div>
                    </div>
                </div>
                ...
                {room.monitoring === 'Camera' && (
                    <div className="hud-card p-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-mono text-slate-400">Camera feed active · Privacy protocols enforced</span>
                        </div>
